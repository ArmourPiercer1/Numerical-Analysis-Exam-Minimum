...
\subsection {逼近准则}
\subsubsection {最小二乘准则（$L_2$范数）}
\subsubsection {一致逼近准则（$L_\infty$范数）}
...
\begin{corollary}[局部收敛判据]
    设 $\mathbf{x}^*$ 是不动点。若 $\rho(\mathbf{\Phi}'(\mathbf{x}^*)) < 1$（谱半径小于1），则不动点迭代 $\mathbf{x}_{k+1} = \mathbf{\Phi}(\mathbf{x}_k)$ 在 $\mathbf{x}^*$ 邻域内局部收敛。
\end{corollary}
...
\begin{itemize}
    ...
    \item $h-\lambda$的复平面区域中，所有使得数值解趋于0的点$(h,\lambda)$的集合称为该方法的绝对稳定域。
...
\begin{property}
    ...
    3. 对于 SPD 矩阵，$\text{cond}_2(A) = \frac{\lambda_{\max}}{\lambda_{\min}}$。
\end{property}
